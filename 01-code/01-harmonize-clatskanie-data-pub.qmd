...
#| include: true
#| label: tbl-ldrd-sc
#| tbl-cap: LDRD soil chemistry data from 2022 SEED soil cores measured in 2024.
#| tbl-column: body
...
: @tbl-ldrd-sc key {#\@tbl-ldrd-sc-key}

# Create GCB Manuscript Analysis Dataset

We stitched together the complete tree, soil and root data set covering 69 trees (24 genotypes), focusing on primarily the 0-15 cm depth for the manuscript analysis. The coding was tricky given we had to update multiple original soil core samples with two differing updates (1/24/24 and 3/24/24) to the SOC fractionation as well as add soil chemistry data taken on 5/24/24. We have excluded Cadmium, Chromium, Molibdinum, and Lead from the root and/or soil chemistry data since many of these were either not measured for both root and soil or they had only trace amounts for many of the samples. The final data set is stored at *./02-data/02-processed/clatskanie-c-fit-data.csv* and described below in @tbl-gcb-data - @tbl-gcb-data-key.
...
#| include: true
#| label: tbl-gcb-data
#| tbl-cap: The final processed data set used for the analysis in the GCB paper.
#| tbl-column: body
...
: @tbl-gcb-data key {#\@tbl-gcb-data-key}

# Supplemental Figures

Additionally, we created supplemental Figures S4 and S6 used in the manuscript below. These files were created here as they relied on soil observations that were not included in the final output data.
