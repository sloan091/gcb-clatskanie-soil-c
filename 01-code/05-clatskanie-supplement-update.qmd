---
title: "Supplemental Information for Clatskanie Manuscript"
...
gc(reset = TRUE)
rm(list = ls())
source("./01-code/01-notebooks/01-data-analysis/fit-helper-fxns.R")
unregister_dopar()
load_libs("ggcorrplot","corrplot", "ggpubr", "ggstatsplot", "ggfortify", "lemon", "fields","ggpmisc", "easystats", "multilevelmod", "broom.mixed")

# Default figure parameters
fig_path <- "04-outputs/02-figures/01-agu/"
fs <- 11
```
...
g1
fig_path_asm <- "./04-outputs/02-figures/04-asm-2024"
ggsave2(file.path(fig_path_asm, "soc-frac.jpg"),
        width = 5.5,
        height = 4)
```
...
dfs <- list(raw = df_fit, log = df_fit_log)
...
H2_results |>
  filter(
    data %in% "raw",
    model %in% "full",
    !grepl("_diff|30", phenotype),
    grepl("MAOM|POM|Tot|BD", phenotype)
  ) |>
  left_join(var_key, by = "phenotype") |>
  ggplot(aes(y = fct_reorder(label, H2),
             x = H2)) +
  geom_col(
    fill = "black",
    position = "dodge",
...
fig_path_asm <- "./04-outputs/02-figures/04-asm-2024"
ggsave2(file.path(fig_path_asm, "H2-C-only.jpg"),
        width = 6,
        height = 6)
```

```{r}
#| label: fig-h2-asm-eff
#| eval: false
#| fig-cap: "Broad sense heritability results for the raw data set."
#| fig-width: 4
#| fig-height: 5
...
fig_path_asm <- "./04-outputs/02-figures/04-asm-2024"
ggsave2(file.path(fig_path_asm, "H2-eff.jpg"),
        width = 5,
        height = 6)
```
...
fig_path <- "./04-outputs/02-figures/03-manuscript"
ggsave2(file.path(fig_path, "fig-s4.jpg"),
        width = 6.5, height = 7)
```
...
cv_fits |>
  ...
  ggridges::geom_density_ridges(scale = 1, rel_min_height = 0,linewidth = 0.2) +
...
cv_fits |>
  ...
  ggridges::geom_density_ridges(scale = 2, rel_min_height = 0,linewidth = 0.01) +
  geom_vline(xintercept = 0, linetype = "solid", linewidth = 0.1) +
  scale_linetype_manual(name = "Predictors",values = c("all" = "dashed", "root" = "solid")) +
  scale_alpha_manual(name = "Predictors", values = c("all" = 0.3, "root" = 1)) +
  #geom_col(position = "dodge", color = "black") +
  xlab("MAOM C Effect [mg C/g soil/SD]") +
  ylab(NULL) +
  #facet_wrap(~outcome, scales = "free_x") +
  theme_cowplot(16) +
  theme(legend.position = "none",
        legend.justification = "center") + 
  guides(fill = guide_legend(title = "Pool"))


ggsave2(file.path(fig_path_asm, "fig-eff.jpg"),
        width = 6.5, height = 6.5)
```
...
fig_path <- "./04-outputs/02-figures/03-manuscript"
ggsave2(file.path(fig_path, "fig-7-gt.pdf"),
        width = 4, height = 3)
...
png(
  ...
  res = 300,
  units = "in",
  file = "04-outputs/02-figures/03-manuscript/fig-s7.jpg",
  type = "cairo"
)
